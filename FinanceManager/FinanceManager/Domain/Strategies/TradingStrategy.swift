...
class TradingStrategy: PFinanceStrategy{
    ...
    func detailedStatistics() -> String{
        if let options = options {
            ...
            let exps = expenses.filter({ $0.category! == "Trading" })
            for expense in exps{
                result += expense.presentInfo()
                if expense.sum >= UserDataManager.data.salary*0.05{
    ...
    func tips() -> String{
        ...
        if !riskyExpenses.isEmpty{
            result += "Theese trades may be too risky:\n\n"
